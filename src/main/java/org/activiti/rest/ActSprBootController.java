...
public class ActSprBootController {
    ...
    public void getBpmnModel() throws IOException {
        List<ProcessDefinition> processDefinitions = repositoryService.createProcessDefinitionQuery()
                .processDefinitionKey("recordProcess").list();
        int bytesWritten = 0;
        int byteCount = 0;
        byte[] bytes = new byte[1024];
        @Cleanup OutputStream outputStream = new FileOutputStream("/home/hrz/Desktop/file-new.png");
        String diagramResourceName = processDefinitions.get(0).getDiagramResourceName();
        @Cleanup InputStream imageStream = repositoryService.getResourceAsStream(
                processDefinitions.get(0).getDeploymentId(), diagramResourceName);
    ...
    public void getPng(@PathVariable("taskId")String taskId) throws IOException {
        ...
        int byteCount = 0;
        byte[] bytes = new byte[1024];
        @Cleanup OutputStream outputStream = new FileOutputStream("/home/hrz/Desktop/file-new2.png");
        while ((byteCount = imageStream.read(bytes)) != -1) {
            outputStream.write(bytes, bytesWritten, byteCount);
            outputStream.flush();
        }
